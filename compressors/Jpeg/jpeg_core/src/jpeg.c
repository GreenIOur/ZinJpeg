...
static void write_DRIinfo(void) {
	...
	writeword(4);  // must be 4?
	writeword(CORRELATED_NUM); // every one block?
}
...
unsigned rst_block_num = 0;
static void block_start(huffman_t *const ctx) {
	if(block_number == 0 || block_number % CORRELATED_NUM != 0)
		return;

	flushbits(&bitbuf);
	ctx[0].dc = 0;
	// 0xFFD0-0xFFD7 = RST marker
	writeword(0xFFD0u | rst_block_num);
	rst_block_num = (rst_block_num+1) % 8;
}
